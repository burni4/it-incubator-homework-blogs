...
export const commentValidationOwnerID = (req: Request, res: Response, next: NextFunction) =>
    body('user').exists({checkFalsy: true}).withMessage('The field [user] must exist')
    .bail().custom(async (value) => {
        const result: boolean = true //await commentsService.checkOwnerComment(value, req.params.id)
        if (!result) {
            res.sendStatus(403);
