...
export const blogTypeValidation = [
    ...
    body('description').trim().exists({checkFalsy: true}).withMessage('The field [Description] must exist')
        .bail().isLength({max: 500}).withMessage('Name length should be max 500 symbols'),
    body('websiteUrl').trim().exists({checkFalsy: true}).withMessage('The field [youtubeUrl] must exist')
